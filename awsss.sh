...
ENCRYPT_KEY=$(openssl rand -base64 32) # Generate random key for encryption
...
print_dashboard() {
    clear
    echo -e "${CYAN}┌──────────────────────────────────────────────────┐${NC}"
    echo -e "${CYAN}│ Ultimate AWS & SMTP Key Leak Detector v3.0       │${NC}"
    echo -e "${CYAN}├──────────────────────────────────────────────────┤${NC}"
...
validate_aws_key() {
    ...
    local secret_key=$2
    local result
    # Simplified check: ensure format and context
    if [[ "$access_key" =~ ^AKIA[0-9A-Z]{16}$ && "$secret_key" =~ ^[0-9a-zA-Z/+]{40}$ ]]; then
        # Optional: Call AWS STS GetCallerIdentity (requires AWS CLI or curl)
        # result=$(AWS_ACCESS_KEY_ID="$access_key" AWS_SECRET_ACCESS_KEY="$secret_key" aws sts get-caller-identity 2>/dev/null)
        # if [ $? -eq 0 ]; then
        #     return 0
        # fi
        # For now, assume valid if format matches and not in comments/examples
        return 0
    fi
    return 1
}
...
find_keys() {
    ...
    echo -e "$valid_aws_access" Oldest: ${valid_aws_access:-}"
